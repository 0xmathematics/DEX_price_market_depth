...
################################
# Query the data
cur.execute("SELECT * FROM uniswap_pool_state_info")
rows = cur.fetchall()
...
new_sql = """   
        ...
        CREATE TABLE IF NOT EXISTS uniswap_pool_state_info (
            ...
            check (char_length(contract_address) = 42),
            UNIQUE (blockchain, project_name, contract_address)   -- combination unique
...
cur.execute('''
            ALTER TABLE uniswap_pool_state_info
...
# Delete table
################################
cur.execute("DROP TABLE IF EXISTS uniswap_pool_state_info CASCADE;") 
